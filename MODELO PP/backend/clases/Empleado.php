...
class Empleado extends Usuario implements ICRUD{
    ...
    public function AgregarF()
    {
        ...
        $usuario="root";
        $retorno=false;
        $pathFoto="./empleados/fotos/".$this->nombre.".".$this->id_perfil.".".date("dmY").".".pathinfo($this->foto,PATHINFO_EXTENSION);
        $this->foto = $pathFoto;
    ...
    public function ModificarF(){
        ...
        unlink($this->foto);
        $pathFoto="./empleados/fotos/".$this->nombre.".".$this->id_perfil.".".date("Gis").".".pathinfo($this->foto,PATHINFO_EXTENSION);
        $this->foto = $pathFoto;
        move_uploaded_file($_FILES["foto"]["tmp_name"], $pathFoto);
